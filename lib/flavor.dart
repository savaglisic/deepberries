...
class _FlavorPageState extends State<FlavorPage> {
  ...
  Future<void> uploadImages() async {
    ...
    for (String imageUrl in capturedImageUrls) {
      String fileName =
          "flavor_${globalState.panelistId}_${globalState
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () async {
                if (flavorRating != null) {
                  await uploadImages();
                  globalState.flavor = flavorRating;
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => SweetnessPage()),
                  );
                } else {
                  showDialog(
                    context: context,
                    builder: (BuildContext context) {
                      return AlertDialog(
                        title: Text("Error"),
                        content: Text(
                            "Please select a flavor rating before continuing."),
                        actions: [
                          TextButton(
                            child: Text("OK"),
                            onPressed: () {
