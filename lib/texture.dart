...
class _TexturePageState extends State<TexturePage> {
  ...
  int? textureClass;
  List<String> textureClassLabels = [
    "Firm",
    "Crunchy",
    "Soft",
    "Mealy",
  ...
  Future<void> uploadImages() async {
    ...
    for (String imageUrl in capturedImageUrls) {
      String fileName =
          "texture_${globalState.panelistId}_${globalState.selectedSample}_${Uuid().v4()}.jpg";
  ...
  Widget build(BuildContext context) {
    ...
    mainAxisAlignment: MainAxisAlignment.center,
      children: [
        ...
        SizedBox(height: 30),
        Text('How do you classify the texture of this sample?',
          style: TextStyle(fontSize: 24, color: Colors.black),
        ),
        Column(
          children: List<Widget>.generate(5, (index) {
            return ListTile(
              title: Text(textureClassLabels[index], style: TextStyle(fontSize: 20)),
              leading: Radio(
                value: index + 1,
                groupValue: textureClass,
                onChanged: (int? value) {
                  setState(() {
                    textureClass = value;
                  });
        ...
        ElevatedButton(
          onPressed: () async {
            if (textureRating != null && textureClass != null) {
              await uploadImages();
              globalState.texture = textureRating;
              globalState.textureClass = textureClassLabels[textureClass! - 1];
              ...
              showDialog(
                ...
                builder: (BuildContext context) {
                  return AlertDialog(
                    title: Text("Error"),
                    content: Text(
                        "Please select a texture rating before continuing."),
                    actions: [
                      TextButton(
                        child: Text("OK"),
                        onPressed: () {
